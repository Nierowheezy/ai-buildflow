...
Read `buildflow/config.json` first. A missing file means the built-in defaults
...
Read `buildflow/context/current-feature.md`. Pull the observable "done when"
...
- All proven -> update only the `**Status:**` line in
  `buildflow/context/current-feature.md` to `verified`, then say it is ready for
  `/complete`.
