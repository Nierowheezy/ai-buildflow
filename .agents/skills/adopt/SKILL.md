---
name: adopt
description: Adopt BuildFlow into an existing brownfield codebase by surveying shipped behavior and generating plans, standards, commands, adapter choices, and visibility setup. Use for /adopt or requests to bootstrap BuildFlow into an established app. Use onboard for a fresh scaffold.
---

# adopt - bootstrap the buildflow from an existing codebase
...
Look at `buildflow/project-plan.md` and `buildflow/build-plan.md`.
...
framework scaffolder (the buildflow is an overlay, never a generator).

Protect the project README:

- If the root `README.md` already looks like a real project README, leave it
  alone.
- If the root `README.md` is the copied BuildFlow workflow doc (for example it
  starts with `# BuildFlow`), report it as obsolete overlay content
  and ask before replacing or removing it. Do not move it into `buildflow/`.
...
- **`buildflow/project-plan.md`** - the what & why, following the existing
  ...
  from the interview.
- **`buildflow/build-plan.md`** - the ordered feature list as a checklist. **Mark
...
- **`buildflow/context/coding-standards.md`** - rewrite the default to match the
...
## Step 5 - ask about BuildFlow visibility

Ask how the BuildFlow workflow files should be handled in git, unless the user
...
- Explain that BuildFlow state, specs, findings, and history will not travel
  ...
  locally.
- Because adoption runs right after the BuildFlow files were added to an
  existing repository, they are more likely to already be staged or committed
  than in a fresh install. If any of `.agents/`, `.claude/`, `buildflow/`, or
...
- BuildFlow visibility choice, and a tracked-file warning if local-only mode was
