...
about whether the BuildFlow is installed correctly, run `/doctor` instead.
...
1. **Project configuration** - read `buildflow/config.json` when present. Report
...
2. **Build plan** - `buildflow/build-plan.md`. Count checked vs unchecked leaf
...
3. **Current work** - `buildflow/context/current-feature.md`. Is something in
...
4. **Findings** - `buildflow/context/findings.md`. Count findings by status and
...
5. **Independent review** - `buildflow/context/review.md`. Report none, pending,
...
6. **Overview freshness** - if `buildflow/context/project-overview.md` is missing,
...
8. **Progress drift** - flag active spec on `main`, a spec in progress but no
   ...
   feature.
9. **Dashboard activity** - read `buildflow/.state/run.json` when it exists.
