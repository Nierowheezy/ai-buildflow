...
`buildflow/context/review.md` holds one active request or latest receipt for the
...
`buildflow/context/current-feature.md` bytes, an ISO-8601 timestamp, and one of
...
- No tracked, staged, unstaged, or untracked path differs from the target except
  `buildflow/context/review.md` and `buildflow/context/findings.md`.
...
The adapter and model fields are declared metadata. BuildFlow proves the target
