...
Read `buildflow/config.json`. A missing file means the built-in defaults apply.
...
Confirm the work is actually finished: `buildflow/context/current-feature.md`
...
Read `buildflow/context/review.md` when present. A missing file on an older
...
- changed files are tied to the active spec, with no unrelated dirty work mixed
  in (dirty `buildflow/context/findings.md` and
  `buildflow/context/review.md` are expected review evidence)
...
- if workflow files changed, `.agents` and `.claude` stayed in sync where both
  adapters exist
- no P0 or P1 finding in `buildflow/context/findings.md` is `open` or `fixed`.
...
- **Feature** - archive `buildflow/context/current-feature.md` to `buildflow/history/features/NN-name.md`
  (NN is the build-plan number), and check it off in `buildflow/build-plan.md`
  (and its parent item once all sub-items are checked).
- **Fix** - archive it to `buildflow/history/fixes/name.md`. A fix isn't a build-plan item, so
  there's nothing to check off.
- **Rollback** - archive it to
  `buildflow/history/rollbacks/YYYY-MM-DD-NN-name.md`, preserving the original
  completed feature archive. Create `buildflow/history/rollbacks/` first if an
  older BuildFlow installation does not have it yet. Uncheck the exact target item in
  `buildflow/build-plan.md` and its parent when applicable, then append a concise
...
**Archive resolved findings.** If `buildflow/context/findings.md` holds any
...
`buildflow/context/review.md`. Preserve the full target and base SHAs, spec
...
Then reset `buildflow/context/review.md` to exactly this stub, creating it when
...
resolved findings, replace `buildflow/context/current-feature.md` with
...
work" stub. Before committing, read the file and confirm it exactly matches:
    ...
    > `buildflow/history/` and resets this file.
...
`buildflow/context/findings.md` exactly matches the canonical Findings stub
above. Otherwise, preserve the remaining entries without rewriting them.
Confirm `buildflow/context/review.md` exactly matches the canonical Independent
Review stub above.
