...
This project is built with the **AI BuildFlow**, a workflow layer, not an
...
framework scaffolder inside a directory that already holds the buildflow files
(`AGENTS.md`, `CLAUDE.md`, `.agents/`, `.claude/`, `buildflow/`); it fails
...
- `buildflow/config.json` - deterministic project workflow settings
- `buildflow/context/project-overview.md` - the project's source of truth
- `buildflow/context/coding-standards.md` - read before changing code
- `buildflow/context/ai-interaction.md` - read when running the BuildFlow workflow
- `buildflow/context/current-feature.md` - the one feature, fix, or rollback being built right now

## Project configuration

`buildflow/config.json` is the user-owned, machine-readable workflow policy for
...
`AGENTS.md` because `CLAUDE.md` imports it. Do not duplicate the same BuildFlow
...
- `onboard` - tune commands, standards, visibility, ignore rules, and tool adapters after overlaying the BuildFlow onto a freshly scaffolded or early project
...
- `doctor` - read-only BuildFlow health check for setup, adapters, plans, overview freshness, and workflow drift
- `adopt` - bootstrap the BuildFlow into an existing brownfield app with shipped features
- `overview` - distill the two planning docs into
  `buildflow/context/project-overview.md`, then offer a reviewed initial planning
...
- `fix` - document an ad-hoc bug or change into `buildflow/context/current-feature.md`
...
- `audit` - branch-aware or full-project review across all concerns or a focused quality, security, performance, or tests lens; `audit independent current` prepares an immutable checkpoint handoff for a selected fresh reviewer session; records findings in `buildflow/context/findings.md` and independent receipts in `buildflow/context/review.md`, where blocking findings or stale review state stop `complete`
...
- `complete` - run the final safety pass, log features, fixes, or rollbacks under `buildflow/history/`, then merge with approval
...
conventions in `buildflow/context/` apply however a step is invoked. `/discovery`
...
The dashboard can show the active or most recent substantial BuildFlow command
from `buildflow/.state/run.json`. This file is generated local state, ignored by
...
checks do not make the BuildFlow unusable.
