...
AI BuildFlow ships workflow files and a dependency-free Node.js installer. The
...
| `npm run sandbox` | Scaffold a minimal app, run the local packed BuildFlow through its real prompts, verify it, and start its server. |
...
| `npm run link:local` | Build this checkout, prepare its template, and link `create-ai-buildflow` and `buildflow` globally so they run from local files. |
...
`npx create-ai-buildflow` always fetches the published package. To install from
...
That builds the installer, prepares its template, and links the `create-ai-buildflow`
and `buildflow` commands globally. They then run entirely from local files, with no
network access:

```bash
create-ai-buildflow --target ../my-app
buildflow status
```
...
`npm run unlink:local` runs `npm rm --global create-ai-buildflow`, which removes any
...
commit, packs the current local BuildFlow, installs that artifact through `npx`,
then runs the app's test and build scripts. The BuildFlow installer stays
...
After BuildFlow installation and verification pass, the command starts the
...
absolute path. It also adds `npm run buildflow:status` and `npm run
buildflow:dashboard` inside that app. Those commands are pinned to the tarball
...
files. They are not copied into applications by `create-ai-buildflow`.
