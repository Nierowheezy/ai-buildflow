...
Read `AGENTS.md`, `buildflow/build-plan.md`,
`buildflow/context/current-feature.md`, the completed feature archives, and git
state.
...
`buildflow/history/features/*.md`. Exclude the directory README.
...
writing or changing `buildflow/context/current-feature.md`. Do not record a
target parent or choose a mainline. Publish `blocked` to
`buildflow/.state/run.json`, explain that BuildFlow cannot safely infer which
...
## Step 2 - separate product changes from BuildFlow history
...
Write `buildflow/context/current-feature.md` using
...
Older installations may not have `buildflow/history/rollbacks/` yet because
...
- does it preserve all BuildFlow history and later plan changes?
...
Format the output to match `buildflow/context/ai-interaction.md`: concise,
