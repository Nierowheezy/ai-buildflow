...
**Target archive:** `buildflow/history/features/NN-name.md`
...
preserving BuildFlow history and compatible work added afterward.
...
- BuildFlow context, adapter skills, rollback spec, and prototypes
...
- [ ] Apply the target commit's product diff in reverse with the Type: Rollback
  ...
  - Done when: the reverse patch applies only to product paths, protected
    BuildFlow paths are unchanged, and the staged diff matches the approved
    rollback scope.
