...
5. After approval, write the plan changes, follow the installed `overview` skill
   to regenerate `buildflow/context/project-overview.md`, then resume this skill
...
`buildflow/context/project-overview.md` before reading it. At or above 20,000
...
`buildflow/context/current-feature.md` (create `buildflow/context/` if needed), following
...
already provided, save it under `buildflow/reference/` (create the folder if
...
- **Flag client vs server** and any conventions from `buildflow/context/coding-standards.md`
...
checked), archive the finished `buildflow/context/current-feature.md` to
`buildflow/history/features/NN-name.md`, then run `/feature` again for the next one.
