...
1. Resume an active feature in `buildflow/context/current-feature.md`.
...
write `buildflow/context/current-feature.md`, and self-review the spec before
...
4. Archive the spec under `buildflow/history/features/`, archive resolved
...
7. Switch to the local default branch, squash-merge the feature branch, and
   create one conventional commit containing product work, tests, and BuildFlow
   history.
...
squash-merge, and delete that fix branch like normal BuildFlow fix work. Re-audit
...
- invalid or conflicting BuildFlow state
...
Follow `buildflow/context/ai-interaction.md`. Keep progress updates concise and
