...
Where this sits in the workflow:
    ...
    (spec     (build it,      (log to buildflow/history/fixes/
...
The input may also be a finding ID from `buildflow/context/findings.md`, alone
...
Pull context from `buildflow/context/project-overview.md` and `buildflow/context/coding-standards.md`,
then write a short spec to `buildflow/context/current-feature.md` (this file holds whatever
is being built now, feature or fix). Keep it lighter than a feature spec:

- **Title** - the bug or change in a few words.
- **Type:** Fix  (so `/complete` logs it to `buildflow/history/fixes/`, not `buildflow/history/features/`).
...
- Keep it small. If it's really a new feature, use `/feature` and the build plan
  instead.
- Same conventions as everything else (`buildflow/context/coding-standards.md`).
