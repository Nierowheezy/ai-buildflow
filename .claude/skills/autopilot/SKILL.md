---
name: autopilot
description: Run one explicit bounded BuildFlow spec and implementation pass through configured quality gates, then stop before completion or external actions. Use only when the user directly invokes /autopilot, $autopilot, or asks for Autopilot.
---

# autopilot - optional BuildFlow loop
...
Autopilot is an explicit opt-in path. It uses the same BuildFlow files and the
...
ask which one should win. Do not overwrite `buildflow/context/current-feature.md`
silently.
...
- `buildflow/config.json` exists but is invalid. Point the user to `/doctor`.
...
The initial `buildflow/.state/run.json` record required by `AGENTS.md` must
...
If `buildflow/context/current-feature.md` already contains an active spec,
...
1. Use the `/feature` behavior for a planned feature, or `/fix` behavior for a
   requested fix.
2. Write `buildflow/context/current-feature.md`.
...
cleanup pass. Findings are recorded in `buildflow/context/findings.md` with
...
- P0/P1 findings still `open` or `fixed` in `buildflow/context/findings.md`,
...
- A P0 or P1 finding left `open` or `fixed` in `buildflow/context/findings.md`
  blocks readiness for `/complete`. The ledger is what makes this enforceable.
- Autopilot stops before `/complete`. It never merges.
- The BuildFlow files remain the state machine. Keep
