---
name: doctor
description: Run a read-only BuildFlow health and context check covering setup, adapters, commands, visibility, plans, overview freshness, configuration, and workflow drift. Use for /doctor, installation checks, context overhead, setup problems, or when something feels wrong.
---

# doctor - BuildFlow health check
...
This skill answers one question: *is this BuildFlow project ready to use?* It is
the diagnostic pass for setup drift, incomplete onboarding, missing files,
placeholder plans, stale generated context, BuildFlow visibility, and confusing
...
1. **Required BuildFlow files**
   - Confirm `AGENTS.md`, `buildflow/`, `buildflow/project-plan.md`,
     `buildflow/build-plan.md`, and `buildflow/context/` exist.
   - Confirm `buildflow/context/coding-standards.md`,
     `buildflow/context/ai-interaction.md`,
     `buildflow/context/current-feature.md`, and
     `buildflow/context/project-overview.md` exist.
   - Confirm `buildflow/history/features/` and `buildflow/history/fixes/` exist.
     When the rollback skill is installed, also check
     `buildflow/history/rollbacks/`. A missing rollback folder on a legacy
     installation is a warning, not a blocker; `/complete` creates it on the
     first rollback.
   - Check `buildflow/context/findings.md`. Missing on a legacy installation is
   ...
   - Check `buildflow/context/review.md`. Missing on a legacy installation is a
   ...
   - If `.gitignore` marks BuildFlow workflow files as local-only, still require
   ...
   - Read `buildflow/config.json` when present. Missing is healthy and means
...
2. **Tool adapters**
   - Read `buildflow/.state/manifest.json` when present and report its exact
   ...
   - If OpenCode is selected, do not require `.opencode/skills/`. If it contains
     duplicate BuildFlow skills alongside `.agents/skills/` or `.claude/skills/`,
...
3. **Commands and project setup**
   - Check whether root `README.md` is still the copied BuildFlow workflow doc
     by looking for `# BuildFlow` or opening text that describes the
     BuildFlow workflow instead of the app. If so, warn that `/onboard` should
     replace it with a project README before publishing.
   - If `buildflow/README.md` clearly contains copied BuildFlow workflow docs,
   ...
   - A missing `Verify` command or GitHub workflow is informational. It means the
     optional automatic-check setup was not selected, not that the BuildFlow is
     unhealthy.
4. **Ignore rules**
   ...
   - Detect local-only BuildFlow mode if `.gitignore` ignores `.agents/`,
     `.claude/`, `buildflow/`, or `CLAUDE.md`. Report it as a visibility choice,
     not a failure, when the local files exist.
   - In local-only mode, check whether tracked `AGENTS.md` still describes the
     BuildFlow workflow, lists hidden adapter paths, or exposes the core skill
...
5. **Planning readiness**
   - Check whether `buildflow/project-plan.md` and `buildflow/build-plan.md` look
     filled in or still template-like. Treat obvious TODO, TBD, example-only text,
     or empty required sections as not ready.
   - Check whether `buildflow/build-plan.md` is a numbered checkbox list. Raw
     bullets are allowed as a first draft, but they should be normalized by
     `/overview` before the build loop starts.
   - Count checked and unchecked leaf items in `buildflow/build-plan.md`.
6. **Overview freshness**
   - Check whether `buildflow/context/project-overview.md` exists and looks
...
7. **Current workflow state**
   - Check whether `buildflow/context/current-feature.md` is the reset stub or an
...
- Required BuildFlow files missing -> overlay the BuildFlow again, or use
  `/adopt` for a brownfield app.
- Invalid `buildflow/config.json` -> fix the named key or value, then rerun
...
- Claude uses the legacy direct context imports -> remove the exact
  `@buildflow/context/coding-standards.md` and
  `@buildflow/context/ai-interaction.md` lines from `CLAUDE.md`, then rerun
  `/doctor`. The files stay in the project and workflow skills still read them.
- Onboarding incomplete -> run `/onboard`.
- Root README is still the BuildFlow workflow doc -> run `/onboard` to replace
  it with a project README before publishing.
- Local-only visibility selected but ignored BuildFlow files are missing ->
  reinstall or restore the BuildFlow files locally.
- Local-only visibility selected but BuildFlow paths are tracked -> ask whether
...
- Plans are placeholders -> fill `buildflow/project-plan.md` and
