...
`/check` proves behavior against the spec. `/doctor` checks BuildFlow setup and
...
one write is the findings ledger at `buildflow/context/findings.md` (Step 4),
the durable record of findings and their status. Independent mode may also
write `buildflow/context/review.md` using the exact record contract in
...
to complete it. BuildFlow verifies the exact target and later staleness. The
...
Use this phase when `buildflow/context/review.md` has no current `pending`
...
2. Read installed adapters from `buildflow/.state/manifest.json` when valid.
...
1. Confirm the current adapter matches `Requested reviewer`, the current model
   ...
   the target except `buildflow/context/review.md` and
   `buildflow/context/findings.md`. Stop on any mismatch or stale state.
...
- `buildflow/context/findings.md`, for existing IDs and statuses
- `buildflow/context/review.md`, for independent request and receipt state
- `buildflow/context/ai-interaction.md`
- `buildflow/build-plan.md`, when feature order matters
...
`buildflow/context/findings.md` is the durable record of findings. Chat reports
