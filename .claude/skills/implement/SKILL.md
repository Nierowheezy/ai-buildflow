...
`buildflow/context/current-feature.md` and stopped.
This skill turns that spec into code, following the build loop in
`buildflow/context/ai-interaction.md`, without vibe coding: small steps, a visible diff plus
...
Read `buildflow/config.json` before changing code. A missing file means the
...
Read `buildflow/context/current-feature.md`. If it has no real spec (still the stub, or its
...
conventions from `buildflow/context/coding-standards.md` and the data model from
`buildflow/context/project-overview.md` so the code matches them.
...
`buildflow/config.json`. The defaults are `feature/<name>` for a feature,
...
commit `git revert`. Completed feature commits also contain BuildFlow history and
...
4. Preview the resolved target's product diff while excluding `.agents/**`, `.claude/**`,
   `buildflow/**`, `AGENTS.md`, `CLAUDE.md`, and
...
5. Apply that resolved product diff in reverse with three-way conflict detection
   and stage it. Use only the resolved full SHA values before running:
       ...
       git diff --binary <target-parent> <target-commit> -- . \
         ':(exclude).agents/**' \
         ':(exclude).claude/**' ':(exclude)buildflow/**' \
...
6. **Mark it done, then prompt when required.** After the applicable gate is
   ...
   `buildflow/context/current-feature.md` so progress survives a context
   clear. If the step repaired a finding tracked in
   `buildflow/context/findings.md`, set that finding's status to `fixed` now too
...
Before handing off, check `buildflow/context/findings.md` and
`buildflow/context/review.md`. A P0 or P1 finding
...
- Iterate on the branch until each step works; never commit code the user hasn't
  approved.
- Follow `buildflow/context/coding-standards.md` (server vs client, scope user-owned queries
