...
> **This buildflow is an overlay layer**, added on top of an already-scaffolded
...
After the first successful `/overview`, BuildFlow offers a reviewed local commit
...
1. **Spec** - Optionally run `/brief` first for a read-only preview of the next
   ...
   @buildflow/context/current-feature.md, then review it together before any code.
...
10. **Safety + log** - `/complete` first checks the active spec, branch, changed
   ...
   workflow files changed. Then it archives the spec to `buildflow/history/features/NN-name.md` (or
   `buildflow/history/fixes/`), checks the feature off in `buildflow/build-plan.md`, and
   resets `buildflow/context/current-feature.md` and
   `buildflow/context/review.md` to their stubs.
...
`buildflow/config.json`. Ask to delete the branch once merged. Continuous Mode's
...
- For visual or replication features (recreating a design, matching a mockup),
  work from a reference image stored in `buildflow/reference/`, not a prose
