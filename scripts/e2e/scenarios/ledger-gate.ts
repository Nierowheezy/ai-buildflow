...
async function run(t: Runner) {
  ...
  t.gitInit();
  t.git("add", "-A");
  t.git("commit", "-m", "chore: fixture app with buildflow");
  const mainBefore = t.git("rev-parse", "main");

  t.git("checkout", "-b", "fix/greeting-punctuation");
  t.write("src/greeting.js", 'exports.greet = () => "Hello, world!";\n');
  t.write("buildflow/context/current-feature.md", FIX_SPEC);
  t.git("add", "-A");
  t.git("commit", "-m", "fix: checkpoint greeting punctuation");
  t.write("buildflow/context/findings.md", OPEN_FINDING);
  ...
  t.check("no fix archive was written", (t.read("buildflow/history/fixes/README.md") !== null) && t.git("status", "--porcelain", "buildflow/history").trim() === "");
  t.check("spec was not reset", (t.read("buildflow/context/current-feature.md") || "").includes("Correct greeting punctuation"));
  t.check("F-01 still open in the ledger", (t.read("buildflow/context/findings.md") || "").includes("[P1] open"));
  ...
  t.write("buildflow/context/findings.md", `${CLOSED_FINDING}\n${UNRESOLVED_FINDINGS}`);
  ...
  const archiveList = t.git("ls-tree", "-r", "--name-only", "main", "buildflow/history/fixes");
  ...
  t.check(
    ...
    normalizeLineEndings(t.read("buildflow/context/current-feature.md")) === CURRENT_FEATURE_STUB
  );
  t.check(
    "unresolved open P2 finding remains unchanged in the ledger",
    normalizeLineEndings(t.read("buildflow/context/findings.md")).includes(
  ...
  t.check(
    ...
    !normalizeLineEndings(t.read("buildflow/context/findings.md")).includes("### F-01") &&
      normalizeLineEndings(t.read("buildflow/context/findings.md")) !== FINDINGS_STUB
  );

  t.phase("lazy-create: /audit rebuilds a deleted ledger in valid format");
  t.git("checkout", "main");
  fs.rmSync(path.join(t.workspace, "buildflow", "context", "findings.md"));
  t.write(
    "src/util.js",
  ...
  const ledger = t.read("buildflow/context/findings.md");
