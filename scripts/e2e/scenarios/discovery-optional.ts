...
async function run(t: Runner) {
  ...
  const projectPlanBefore = t.read("buildflow/project-plan.md");
  const buildPlanBefore = t.read("buildflow/build-plan.md");
  const overviewBefore = t.read("buildflow/context/project-overview.md");
  ...
  t.check("project plan stayed unchanged", t.read("buildflow/project-plan.md") === projectPlanBefore);
  t.check("build plan stayed unchanged", t.read("buildflow/build-plan.md") === buildPlanBefore);
  t.check("overview stayed unchanged", t.read("buildflow/context/project-overview.md") === overviewBefore);
  t.check("discovery left the working tree clean", t.git("status", "--porcelain") === "");

  t.phase("manual plans continue directly to overview");
  t.write("buildflow/project-plan.md", PROJECT_PLAN);
  t.write("buildflow/build-plan.md", BUILD_PLAN);
  t.git("add", "-A");
  ...
  t.check(
    ...
    /reading queue/i.test(t.read("buildflow/context/project-overview.md") || "")
  );
  t.check("manual project plan stayed unchanged", t.read("buildflow/project-plan.md") === PROJECT_PLAN);
  t.check("manual build plan stayed unchanged", t.read("buildflow/build-plan.md") === BUILD_PLAN);
  t.check(
    "only the generated overview changed",
    changedPaths.length === 1 && changedPaths[0] === "buildflow/context/project-overview.md"
  );
}
