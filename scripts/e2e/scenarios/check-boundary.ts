...
async function run(t: Runner) {
  t.phase("setup");
  t.installBuildFlow();
  t.write("buildflow/context/current-feature.md", FEATURE_SPEC);
  ...
  const specBefore = t.read("buildflow/context/current-feature.md");
  ...
  t.check(
    ...
    t.read("buildflow/context/current-feature.md") === specBefore
  );
