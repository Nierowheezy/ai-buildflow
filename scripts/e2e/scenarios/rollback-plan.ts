...
async function run(t: Runner) {
  t.phase("setup");
  t.installBuildFlow();
  t.write("buildflow/build-plan.md", "# Build Plan\n\n- [ ] 1. Greeting punctuation\n");
  ...
  t.write("buildflow/history/features/01-greeting-punctuation.md", FEATURE_ARCHIVE);
  t.write("src/greeting.js", 'console.log("Hello, world!");\n');
  t.git("add", "-A");
  ...
  const archiveBefore = t.read("buildflow/history/features/01-greeting-punctuation.md");
  ...
  const rollbackSpec = t.read("buildflow/context/current-feature.md") || "";
  ...
  t.check(
    ...
    t.read("buildflow/history/features/01-greeting-punctuation.md") === archiveBefore
  );
  ...
  t.check(
    ...
    changedPaths.length === 1 && changedPaths[0] === "buildflow/context/current-feature.md"
  );

  t.phase("rollback blocks a merge target during planning");
  t.git("restore", "buildflow/context/current-feature.md");
  t.write(
  ...
  t.write("buildflow/history/features/02-merge-target.md", MERGE_FEATURE_ARCHIVE);
  t.git("add", "-A");
  ...
  const currentFeatureBefore = t.read("buildflow/context/current-feature.md") || "";
  const mergeArchiveBefore = t.read("buildflow/history/features/02-merge-target.md") || "";
  ...
  t.check(
    ...
    (t.read("buildflow/context/current-feature.md") || "") === currentFeatureBefore
  );
  t.check(
    "merge target planning leaves the archive unchanged",
    (t.read("buildflow/history/features/02-merge-target.md") || "") === mergeArchiveBefore
  );
  t.check(
