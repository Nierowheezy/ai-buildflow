...
class Runner {
  ...
  installBuildFlow(adapterFlag = getDefaultAdapterFlag()): void {
  ...
  gitInit(): void {
    ...
    run("git", ["config", "user.email", "e2e@ai-buildflow.test"], this.workspace);
    run("git", ["config", "user.name", "BuildFlow E2E"], this.workspace);
  }
