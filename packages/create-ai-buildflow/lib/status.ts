...
const PROJECT_PLAN_PATH = path.join("buildflow", "project-plan.md");
const BUILD_PLAN_PATH = path.join("buildflow", "build-plan.md");
const AGENTS_PATH = "AGENTS.md";
const ONBOARDING_SENTINEL = "<!-- buildflow:onboarding-required -->";
...
const OVERVIEW_SOURCE_HASH_PATTERN =
  /<!--\s*buildflow:source-hash\s+([a-f0-9]{64})\s*-->/i;
...
function formatHumanStatus(
  ...
  const adapters = status.buildflow.adapters.length > 0
    ? status.buildflow.adapters.join(", ")
    : "none detected";
  const lines = [
    `${style.bold(style.cyan("BuildFlow Status"))}  ${style.bold(status.project.name)}`,
    "",
    formatSection("Project", style),
    formatRow("Path", status.project.root, style),
    formatRow("Version", status.buildflow.version || "unknown", style),
...
function selectNextAction(
  ...
  if (onboarding.state === "unknown") {
    return {
      command: "/doctor",
      reason: onboarding.reason || "Confirm BuildFlow onboarding state before continuing."
    };
  }
...
async function readOnboardingStatus(projectRoot: string): Promise<{
  ...
  try {
    ...
    if (
      ...
      const reason = "BuildFlow onboarding has not tuned the project commands yet.";
      return {
