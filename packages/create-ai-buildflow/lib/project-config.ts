...
// path.join or Windows would surface "buildflow\config.json" to users.
const PROJECT_CONFIG_PATH = "buildflow/config.json";
...
async function readProjectConfig(projectRoot: string): Promise<ProjectConfigResult> {
  ...
  try {
    ...
    if (stats.isSymbolicLink()) {
      return invalidResult("BuildFlow config is a symbolic link and was not read.");
    }

    if (!stats.isFile()) {
      return invalidResult("BuildFlow config path is not a regular file.");
    }
    ...
    try {
      ...
      if (error instanceof SyntaxError) {
        return invalidResult(`Invalid BuildFlow config JSON: ${PROJECT_CONFIG_PATH}`);
      }
    ...
    try {
      ...
      return invalidResult(
        `Unsupported or invalid BuildFlow config: ${PROJECT_CONFIG_PATH}. ${error instanceof Error ? error.message : String(error)}`
      );
    }
  } catch (error: unknown) {
    ...
    return invalidResult(
      `Unable to read BuildFlow config: ${error instanceof Error ? error.message : String(error)}`
    );
  }
