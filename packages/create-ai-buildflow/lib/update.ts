...
const CONTROL_DIR = "buildflow/.state";
...
const RETIRED_MANAGED_PATHS = new Set(["buildflow/README.md"]);
...
async function readManifest(targetDir: string): Promise<Manifest | null> {
  ...
  try {
    ...
    if (error instanceof SyntaxError) {
      throw new Error(`Invalid BuildFlow manifest JSON: ${MANIFEST_PATH}`);
    }
...
function validateManifest(manifest: unknown): asserts manifest is Manifest {
  ...
  if (
    ...
    throw new Error(`Unsupported or invalid BuildFlow manifest: ${MANIFEST_PATH}`);
  }
}
...
async function prepareUpdate({
  ...
  if (adapters.length === 0) {
    throw new Error(
      "No installed BuildFlow adapter skills were found in the target directory."
    );
  }
...
async function applyPreparedUpdate(
  ...
  for (const operation of [...plan.add, ...replacements]) {
    ...
    if ((await hashFile(stageFile)) !== operation.desired.hash) {
      throw new Error(
        `BuildFlow template changed after the update plan was created: ${operation.path}`
      );
    }
  }
  ...
  try {
    ...
    try {
      ...
      throw new Error(
        `BuildFlow update failed: ${getErrorMessage(error)}. Rollback also failed: ${getErrorMessage(rollbackError)}`
      );
    }

    throw new Error(`BuildFlow update failed and was rolled back: ${getErrorMessage(error)}`);
  } finally {
...
async function atomicWrite(target: string, content: string | Uint8Array): Promise<void> {
  ...
  const temporary = path.join(
    path.dirname(target),
    `.${path.basename(target)}.buildflow-${process.pid}-${crypto.randomBytes(4).toString("hex")}`
  );
...
function normalizeRelativePath(relativePath: string): string {
  ...
  if (!isSafeRelativePath(normalized)) {
    throw new Error(`Unsafe BuildFlow path: ${relativePath}`);
  }
