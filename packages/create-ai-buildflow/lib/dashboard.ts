...
async function startDashboardServer(
  ...
  if (!address || typeof address === "string") {
    ...
    throw new Error("BuildFlow dashboard could not determine its local address.");
  }
  ...
  if (pathname === "/api/status") {
    try {
      ...
      sendResponse(
        ...
        "application/json; charset=utf-8",
        `${JSON.stringify({
          error: error instanceof Error ? error.message : "Unable to read BuildFlow status."
        })}\n`
      );
    }
    return;
...
<head>
  ...
  <title>BuildFlow Dashboard</title>
  <style>
    :root {
      color-scheme: light;
...
<body>
  <main class="shell">
    ...
    <section class="dashboard-grid" aria-label="BuildFlow project status">
      <div class="main-column">
        ...
        <article class="card history-card">
          ...
          <ul class="timeline" id="history-list" tabindex="0" aria-label="Completed BuildFlow work"><li class="empty">Loading completed work...</li></ul>
        </article>
      </div>
      ...
      <aside class="status-rail" aria-label="Project state">
        <article class="card project-state-card">
          ...
          <div class="fact-group">
            <div class="fact-group-label">BuildFlow</div>
            <div class="facts">
    ...
    <footer>Read-only local dashboard. Updates as BuildFlow and project files change.</footer>
  </main>
  ...
  <script>
    ...
    function render(status) {
      ...
      byId("version").textContent = status.buildflow.version || "unknown";
      byId("adapters").textContent = status.buildflow.adapters.join(", ") || "none detected";
