...
} from "../bin/create-ai-buildflow.js";
import {
  CONTROL_DIR,
  MANIFEST_PATH,
  adapterListFromMode,
...
test("shared adapters reuse compatible skill trees", () => {
  ...
  assert.deepEqual(
    ...
    ["AGENTS.md", "buildflow", "CLAUDE.md", ".claude"]
  );
  assert.deepEqual(
    getTemplateEntries(["codex", "claude", "copilot", "opencode"]).map(
      (entry) => entry.target
    ),
    ["AGENTS.md", "buildflow", ".agents", "CLAUDE.md", ".claude"]
  );
...
test("global CLI installation is offered after interactive installs and updates", () => {
  ...
  assert.equal(
    ...
    "npm install --global create-ai-buildflow@0.9.0"
  );
  assert.equal(
    ...
    "\nInstall the optional global `buildflow` CLI command?\n" +
      ...
      "This runs: npm install --global create-ai-buildflow@0.12.0\n" +
      "Continue? [y/N]: "
  );
  assert.equal(
    getGlobalCliPrompt("update", "0.11.1", "0.12.0"),
    "\nUpdate the optional global `buildflow` CLI from 0.11.1 to 0.12.0?\n" +
      "This adds the shorter `buildflow status` and `buildflow dashboard` commands.\n" +
      "Without it, use:\n" +
      "  npx create-ai-buildflow@latest status\n" +
      "  npx create-ai-buildflow@latest dashboard\n" +
...
test("new installs record only BuildFlow-owned managed files", async (t) => {
  ...
  await writeFiles(targetDir, {
    ...
    "buildflow/build-plan.md": "Project roadmap\n"
  });
  ...
  await assert.rejects(fs.access(path.join(targetDir, ".ai-buildflow")), {
    code: "ENOENT"
  });
  assert.equal(
    await fs.readFile(path.join(targetDir, "buildflow/build-plan.md"), "utf8"),
    "Project roadmap\n"
  );
});
...
test("update replaces unchanged managed files and preserves project files", async (t) => {
  ...
  await writeFiles(targetDir, {
    ...
    "buildflow/build-plan.md": "Custom roadmap\n",
    "buildflow/config.json": "{\"schemaVersion\":1}\n",
    "buildflow/context/decisions.md": "Keep this decision\n"
  });
  ...
  assert.match(
    path.relative(resolvedTarget, result.backupDir).replaceAll(path.sep, "/"),
    /^buildflow\/\.state\/backups\/2026-07-15T12-00-00Z-1\.0\.0-to-1\.1\.0-[a-f0-9]{8}$/
  );
  assert.ok(result.backupDir);
  assert.equal(
  ...
  assert.equal(
    await fs.readFile(path.join(targetDir, "buildflow/config.json"), "utf8"),
    "{\"schemaVersion\":1}\n"
  );
  assert.equal(
    await fs.readFile(path.join(targetDir, "buildflow/context/decisions.md"), "utf8"),
    "Keep this decision\n"
  );
  assert.equal(
    await fs.readFile(
...
test("update removes an unchanged BuildFlow README installed by an older version", async (t) => {
  const workspace = await createWorkspace(t);
  const templateRoot = path.join(workspace, "template");
  const targetDir = path.join(workspace, "target");
  const readmeContent = "Legacy BuildFlow docs\n";
  const files = {
    ".agents/skills/check/SKILL.md": "Check skill\n"
  };
  ...
  await writeFiles(targetDir, {
    ...files,
    "buildflow/README.md": readmeContent
  });
  await writeInstallManifest({
    targetDir,
    templateRoot,
  ...
  assert.ok(result.backupDir);
  await assert.rejects(fs.access(path.join(targetDir, "buildflow/README.md")), {
    code: "ENOENT"
  });
});

test("update preserves a locally modified legacy BuildFlow README", async (t) => {
  const workspace = await createWorkspace(t);
  const templateRoot = path.join(workspace, "template");
  const targetDir = path.join(workspace, "target");
  const readmeContent = "Legacy BuildFlow docs\n";
  ...
  await writeFiles(targetDir, {
    "buildflow/README.md": "Locally customized docs\n"
  });
  ...
  assert.equal(
    await fs.readFile(path.join(targetDir, "buildflow/README.md"), "utf8"),
...
test("legacy installs treat differing managed files as conflicts", async (t) => {
  ...
  await writeFiles(targetDir, {
    "buildflow/README.md": "Current BuildFlow docs\n",
...
test("failed apply removes additions and restores the previous manifest", async (t) => {
  ...
  fs.rename = async (source, target) => {
    if (!injectedFailure && target === path.join(resolvedTargetDir, MANIFEST_PATH)) {
  ...
  try {
    await assert.rejects(
      applyPreparedUpdate(prepared),
      /BuildFlow update failed and was rolled back/
    );
  } finally {
...
async function createWorkspace(t: TestContext): Promise<string> {
  const workspace = await fs.mkdtemp(path.join(os.tmpdir(), "ai-buildflow-update-"));
