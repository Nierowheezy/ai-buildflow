...
test("dashboard serves live read-only project status on loopback", async (t) => {
  ...
  assert.match(page, /BuildFlow Dashboard/);
  ...
  assert.match(page, /id="history-list" tabindex="0" aria-label="Completed BuildFlow work"/);
...
`
  ...
  await fs.writeFile(
    path.join(projectRoot, "buildflow", "context", "current-feature.md"),
    `# Feature: Dashboard
...
`
  ...
  await fs.writeFile(
    path.join(projectRoot, "buildflow", "history", "features", "02-dashboard.md"),
    "# Feature: Dashboard\n\n**From build-plan:** feature 2\n**Status:** complete\n"
  );
...
test("dashboard emits an immediate refresh event when project state changes", async (t) => {
  ...
  await fs.appendFile(
    path.join(projectRoot, "buildflow", ".state", "run.json"),
    "\n"
  );
...
async function createProject(t: TestContext): Promise<string> {
  const workspace = await fs.mkdtemp(path.join(os.tmpdir(), "buildflow-dashboard-"));
  ...
  const historyRoot = path.join(projectRoot, "buildflow", "history", "features");
